...
if __name__=='__main__':
    ...
    input_size  = (32, 32, 3)
    vgg16_model = VGG16_Model(input_size  = input_size,
                          num_classes = num_classes,
                          train_info  = train_info)
    
    train_vgg16_model =  vgg16_model.model_fit()

    # D:\upgraded_git_repo\test_info
    vgg16_model.save_model(model            = train_vgg16_model,
                         json_file_path   = r'D:\upgraded_git_repo\test_info\vgg16.json',
                         weight_file_path = r'D:\upgraded_git_repo\test_info\vgg16.h5')
    
    mapping = dict()
    for number, categories in zip(np.arange(0,53),encoder_info.inverse_transform(np.arange(0,53))):
        mapping[str(number)]=categories

    with open(r"D:\upgraded_git_repo\test_info\label_encoder_VGG16.json", "w") as json_file:
